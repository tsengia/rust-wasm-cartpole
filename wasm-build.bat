...
mkdir .\src\wasm\simd\
set RUSTFLAGS=-C embed-bitcode=yes -C target-feature=+simd128,+atomics,+bulk-memory,+mutable-globals --cfg web_sys_unstable_apis --cfg getrandom_backend="wasm_js"
echo "Running cargo build for SIMD"
cargo build --target wasm32-unknown-unknown --target-dir target-simd || exit 1
wasm-bindgen.exe --target web --typescript --out-dir .\src\wasm\simd\ .\target-simd\wasm32-unknown-unknown\debug\burn_polecart_wasm.wasm
...
set RUSTFLAGS=-C embed-bitcode=yes -C target-feature=+atomics,+bulk-memory,+mutable-globals --cfg web_sys_unstable_apis --cfg getrandom_backend="wasm_js"
mkdir .\src\wasm\no-simd\
echo "Running cargo build for non-SIMD"
cargo build --target wasm32-unknown-unknown --target-dir target || exit 1
wasm-bindgen.exe --target web --typescript --out-dir .\src\wasm\no-simd\ .\target\wasm32-unknown-unknown\debug\burn_polecart_wasm.wasm
